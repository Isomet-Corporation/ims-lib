...
CCyUSBDevice::~CCyUSBDevice(void) noexcept(false)
{
    ...
    if (hDevNotification) {
        if (! UnregisterDeviceNotification(hDevNotification))
            throw L"Failed to close the device notification handle.";
            ...
            deviceNumber, &devInterfaceData)) {
                ...
                if (SetupDiGetInterfaceDeviceDetail (hwDeviceInfo,
                    ...
                    &devInfoData)) {
                        ...
                        int pathLen = requiredLength - (sizeof (functionClassDeviceData->cbSize)+sizeof (functionClassDeviceData->DevicePath));
                        //int pathLen = requiredLength - functionClassDeviceData->cbSize;

						memset((void *)DevPath, '\0', sizeof(DevPath[0]) * USB_STRING_MAXLEN);
